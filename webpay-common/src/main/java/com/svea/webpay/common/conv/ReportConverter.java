...
public class ReportConverter {
	...
	public static String mapPaymentReportPaymentType(String clientReportPaymentType) {
		...
		switch (clientReportPaymentType) {
			...
			case ClientReportRow.PAYMENTTYPE_Swish:
				result = SveaCredential.ACCOUNTTYPE_SWISH;
				break;
				
			case ClientReportRow.PAYMENTTYPE_Vipps:
				result = SveaCredential.ACCOUNTTYPE_VIPPS;
				break;
			...
			if (cr.getRows()!=null && cr.getRows().size()>0) {
				...
				for (ClientReportRow crr : cr.getRows()) {
					...
					if (paymentTransactionId!=null) {
						d.setPaymentId(paymentTransactionId.toString());
					}
					...
					if (crr.getPaymentType()!=null && crr.getPaymentType().trim().length()>0) {
						d.setPaymentType(mapPaymentReportPaymentType(crr.getPaymentType()));
					}
